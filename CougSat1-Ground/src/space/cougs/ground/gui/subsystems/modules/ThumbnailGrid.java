...
public class ThumbnailGrid extends JComponent {
  ...
  private final List<ActionListener> actionListners =
      ...
      new MouseMotionListener() {
        ...
        public void mouseDragged(MouseEvent e) {
          if (scrollBar.contains(e.getPoint())) {
            ...
            y = (int)Math.max(
                0, Math.min(y, getHeight() - scrollBar.getHeight()));
            scrollBar.y = y;
  ...
  public void paintComponent(Graphics g) {
    ...
    gridHeight =
        (int)(squareLength *
              Math.max(1, Math.ceil((double)thumbnails.size() / columns)));
    scrollBar.setSize(barWidth, getHeight() * getHeight() / gridHeight);
    scrollBar.x = squareLength * 2;

    int y = (int)(-squareLength -
                  scrollPosition * Math.max(0, gridHeight - this.getHeight()));
    for (int i = 0; i < thumbnails.size(); i++) {
      ...
      g2d.drawString(
          String.valueOf(i), x + squareLength / 2, y + squareLength / 2);
    }

    g2d.setColor(CustomColors.BAR_BACKGROUND);
    g2d.fillRect(squareLength * 2, 0, this.getWidth() - squareLength * 2,
        this.getHeight());
